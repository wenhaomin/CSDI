...
class CleanDataset():
    ...
    def read_data(self):
        if 'PEMS' in self.data_name:
            ...
            data = np.expand_dims(np.load(self.feature_file)[:, :, 0], -1)
            data = np.nan_to_num(data, nan=0) # todo: think what should be put here
        else:
...
class TrafficDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        start, end = recent_idx[1][0], recent_idx[1][1]
        label = self.label[start:end] # (t)

        start, end = recent_idx[0][0], recent_idx[0][1]
        node_feature = self.feature[start:end] # (T_h, V, 1)
        pos_w, pos_d = self.get_time_pos(start)
        pos_w = np.array(pos_w, dtype=np.int32)
        pos_d = np.array(pos_d, dtype=np.int32)
...
def get_dataloader(batch_size, device, validindex=0):
    ...
    # scaler = torch.from_numpy(clean_data.std).to(device).float()  # 方差
    # mean_scaler = torch.from_numpy(clean_data.mean).to(device).float()


    return train_loader, val_loader, test_loader, scaler, mean_scaler
